...
export function parseMealText(input, foods) {
  ...
  const rawSegments = text.split(/[、，,。；;]|(?:和|加)/).map(function (part) { return part.trim() }).filter(Boolean)
  ...
  return result(unknown.length ? 'needs_review' : 'complete', recognized, unknown)
}

function result(status, recognizedItems, unknownSegments) {
  return {
    status: status,
