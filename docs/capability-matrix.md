...
| 一句话记餐与食品目录 | P0，已实现 | 一句话记餐为默认入口，60 项本地食品及冻结解析样例通过自动测试；2026-09-12 已在比赛健康镜像实测示例识别并进入确认页，未知内容明确待处理 | 最近项、收藏和手动目录位于备用的“快捷选择”；待处理内容禁止静默保存 | 是；自动测试与改版运行验收已通过 |
...
| goldfish RPK | P0 部署门禁 | production RC1 已解包推送并由 `vapp` 启动；日志确认包名/版本、storage、VelaClaw 与 index ready；运行时未注册 `system.router`，故根 URI 使用 index 验证降级页 | index 明示环境边界并提供 VelaClaw 无 Key 降级验证；完整七页流程以 AIoT 比赛模拟器为准 | 是；production 启动已通过 |
...
- RC1 production RPK 已在 AIoT 比赛健康镜像完成完整业务验收，并在 goldfish 完成解包、推送和 index 启动；初赛不要求真机，本结论也不外推为真机验证。
...
- 视觉以比赛镜像的 466×466 圆屏为优先验收环境：深色表盘背景、暖橙主操作、绿色完成状态、大数字、圆角卡片；480×480、方屏和窄屏仍需在发布冻结前完成不溢出复核。
